...
function Contacts() {
  ...
  return (
    <>
      <div className='flex bg-[#ffffff] flex-col -space-y-1 overflow-y-scroll scrollbar-hide h-[87vh] pb-10'>
        {
          chats?.length > 0 ? chats?.map((e) => {
            return (
              <div onClick={() => {
                dispatch(setActiveChat(e))
              }} key={e._id} className={`flex mb-2 mt-px items-center border border-gray-200 justify-between sm:gap-x-1 rounded-xl mx-3 md:gap-x-1 ${activeChat._id === e._id ? "bg-slate-200" : "bg-[#ffffff]"} cursor-pointer  py-4 px-2`}>
                <div className='flex items-center gap-x-3 sm:gap-x-1 md:gap-x-3'>
                  <img className='w-12 h-12  sm:w-12 sm:h-12 rounded-[30px] object-cover' src={getChatPhoto(e, activeUser)} alt="" />
                  <div>
